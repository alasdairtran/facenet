...
meanImage19 = np.array([103.939, 116.779, 123.68]).reshape(
    (1, 1, 1, 3))  # pylint: disable=no-member
...
def sqErrorLossStyle(sess, modelGraph):
    def intermediateCalc(x, y):
        ...
        A = tf.matmul(tf.transpose(tf.reshape(x, (M, N))),
                      tf.reshape(x, (M, N)))
        G = tf.matmul(tf.transpose(tf.reshape(y, (M, N))),
                      tf.reshape(y, (M, N)))
        return (1 / (4 * N**2 * M**2)) * tf.reduce_sum(tf.pow(G - A, 2))
    E = [intermediateCalc(sess.run(modelGraph[layerName]),
                          modelGraph[layerName]) for layerName, _ in styleLayers]
...
# Addition of extra dimension to image
paintingStyleImage = np.reshape(
    paintingStyleImage, ((1,) + paintingStyleImage.shape))
paintingStyleImage = paintingStyleImage - meanImage19
# Display image
pyplot.imshow(paintingStyleImage[0])

imageNoise = np.random.uniform(-20, 20, (1,
                                         outputHeight, outputWidth, 3)).astype('float32')
