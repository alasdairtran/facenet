...
def run_train():
    ...
    with tf.Graph().as_default():
        ...
        images_placeholder = tf.compat.v1.placeholder(tf.float32, shape=(
            FLAGS.batch_size, FLAGS.image_size, FLAGS.image_size, 3), name='input')
...
def inference_conv_test(images):
    ...
    # Affine layer not needed to reproduce the error
    affn = _affine(resh1, 147456, 128)
    return affn
...
assert all(
    all_vars_close), 'Variable values differ between the two sessions (with the same seed)'
assert all(
    all_grads_close), 'Gradient values differ between the two sessions (with the same seed)'
